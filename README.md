...
راجع الوثيقة التنفيذية الكاملة:

```text
docs/SPEC.md
```

تشمل: خريطة الشاشات، نماذج إبلاغ/اقتراح، دخول الموظف، API، مخطط قاعدة البيانات، الأمن، الإشعارات، خطة الاختبار، ومراحل التسليم.

## التدفق الحالي في الواجهة

1. Splash → ابدأ  
2. الرئيسية → بطاقة مشروع وِجهة الشمال  
3. صفحة المشروع → إبلاغ / اقتراح / دخول الموظف / متابعة  
4. إرسال النموذج → رقم تذكرة → تتبع + لوحة موظف
